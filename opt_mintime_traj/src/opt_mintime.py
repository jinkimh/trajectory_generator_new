...
def opt_mintime(reftrack: np.ndarray,
    ...
    if pars["optim_opts"]["safe_traj"]:
        if (pars["optim_opts"].get("ax_pos_safe") is None
                or pars["optim_opts"].get("ax_neg_safe") is None
                or pars["optim_opts"].get("ay_safe") is None):
            ggv_path = pars.get("ggv_file")
            axm_path = pars.get("ax_max_machines_file")
            if ggv_path is None:
                raise ValueError("safe_traj requires ax_pos_safe/ax_neg_safe/ay_safe or ggv_file in pars")
            ggv, _ = tph.import_veh_dyn_info.import_veh_dyn_info(
                ggv_import_path=ggv_path,
                ax_max_machines_import_path=axm_path
            )
            if pars["optim_opts"].get("ax_pos_safe") is None:
                pars["optim_opts"]["ax_pos_safe"] = np.amin(ggv[:, 1])
            if pars["optim_opts"].get("ax_neg_safe") is None:
                pars["optim_opts"]["ax_neg_safe"] = -np.amin(ggv[:, 1])
            if pars["optim_opts"].get("ay_safe") is None:
    ...
    def _to_float(v):
        if isinstance(v, (float, int, np.floating)):
            return float(v)
        if isinstance(v, (ca.DM, ca.SX, ca.MX)):
            try:
                return float(v)
            except Exception:
                arr = np.array(v.full()).reshape(-1)
                if arr.size == 1:
                    return float(arr[0])
                return arr
    ...
    def _as_flat_list(x):
        if isinstance(x, (list, tuple)):
            flat = []
            for v in x:
                flat.extend(_as_flat_list(v))
            return flat
        v = _to_float(x)
        if isinstance(v, np.ndarray):
            return [float(i) for i in v.tolist()]
        return [float(v)]
    ...
    def _flat(seq):
        ...
        for i, x in enumerate(seq):
            try:
                vals = _as_flat_list(x)
                arr = np.array(vals, dtype=float).reshape(-1)
            except Exception as exc:
                raise ValueError(f"flat_fail idx={i} type={type(x)} value={x}") from exc
            out.append(arr)
    ...
    opts = {
        "expand": True,
        "verbose": print_debug,
        "ipopt.max_iter": pars["optim_opts"].get("ipopt_max_iter", 2000),
        "ipopt.tol": pars["optim_opts"].get("ipopt_tol", 1e-7),
        "ipopt.print_level": pars["optim_opts"].get("ipopt_print_level", 5),
        "print_time": pars["optim_opts"].get("ipopt_print_time", False),
    }
    output_file = pars["optim_opts"].get("ipopt_output_file")
    if output_file:
        opts["ipopt.output_file"] = output_file
        opts["ipopt.file_print_level"] = pars["optim_opts"].get(
            "ipopt_file_print_level",
            opts["ipopt.print_level"],
        )
    ...
    if pars["optim_opts"]["warm_start"]:
        try:
            ...
            raise RuntimeError("Failed to load warm start files for mintime")

    # check warm start files
    if pars["optim_opts"]["warm_start"] and not len(w0) == len(lbw):
        raise RuntimeError("Warm start files do not fit to the dimension of the NLP")
    ...
    if solver.stats()['return_status'] != 'Solve_Succeeded':
        raise RuntimeError(f"Optimization failed: {solver.stats()['return_status']}")
